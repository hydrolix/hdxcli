...
from hdx_cli.library_api.common.exceptions import BadFileNameConventionException, HdxCliException
...
def _create_cluster_config_file_for_tests():
    if not HDXCLI_TESTS_CLUSTER_USERNAME or not HDXCLI_TESTS_CLUSTER_HOSTNAME:
        raise RuntimeError(
            "HDXCLI_TESTS_CLUSTER_USERNAME and HDXCLI_TESTS_CLUSTER_HOSTNAME"
            " environment vars must be set"
        )
    tests_protocol = "http" if HDXCLI_TESTS_CLUSTER_SSL_ACTIVE == "0" else "https"
    config = {"default": {"hostname": HDXCLI_TESTS_CLUSTER_HOSTNAME, "scheme": tests_protocol}}
...
HDXCLI_PROFILE_DATA = toml.load(open(HDXCLI_PROFILE_DATA_FILE, "r", encoding="utf-8"))
...
def _load_toml_file_with_extension(toml_file_path, expected_extension=""):
    if not (fname := Path(toml_file_path).name).endswith(expected_extension):
        raise BadFileNameConventionException(
            f"Filename with ending in {expected_extension}" f" expected. Got {fname}"
        )
...
def _add_parameters_to_hdx_cli_command(cmd: str):
    ...
    if HDXCLI_TESTS_CLUSTER_USERNAME:
        idx += 1
        split_cmd.insert(idx, "--username")
        idx += 1
        split_cmd.insert(idx, HDXCLI_TESTS_CLUSTER_USERNAME)
...
class ApprovalTestCaseRunner:
    def __init__(
        ...
        name,
        input_commands,
        expected_output_tpl: Tuple[ExpectedOutput, str],
        *,
        setup_steps=None,
        teardown_steps=None,
    ):
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"Assertion error in test '{self._name}', Command: '{command_capture}'",
                file=sys.stderr,
            )
...
def test_approval_run_all(test_data: Tuple[List[str], str, str]):
    ...
    tc_runner = ApprovalTestCaseRunner(
        name=test_data[2],
...
def pytest_generate_tests(metafunc):
    ...
    if global_setup:
        global_setup_interpolated = [interpolate_with_profile_vars(tic) for tic in global_setup]
    ...
    for tst in tests_array:
        test_input_commands = [
            interpolate_with_profile_vars(tic) for tic in tst["commands_under_test"]
        ]
        setup_commands = None
        if tst.get("setup") is not None:
            setup_commands = [interpolate_with_profile_vars(tic) for tic in tst.get("setup")]

        all_tests.append(
            (
                test_input_commands,
                _parse_expected_output(tst),
                tst["name"],
                setup_commands,
