...
# - input_commands: an input command, specified as a toml list
#
# - one of (expected_output, expected_output_re, expected_output_expr): it checks if the result is correct
#   according to the following logic:
#
#   1. if expected_output is used, the output is compared verbatim against input_commands command
...
#
# - setup (optional): a setup list of commands executed before running the command
# - teardown (optional): list with teardown commands
global_setup = ["python3 -m hdx_cli.main project create test_ci_project",
                "python3 -m hdx_cli.main --project-name test_ci_project table create test_ci_table",
                "python3 -m hdx_cli.main --project-name test_ci_project --table-name test_ci_table transform create -f test_ci_transform.json test_ci_transform"]
...
input_commands = ["python3 -m hdx_cli.main --project-name test_ci_project table list"]
# Only last command output is checked
...
input_commands = ["python3 -m hdx_cli.main --project-name test_ci_project table delete --disable-confirmation-prompt a_table_that_does_not_exist"]
# Only last command output is checked
expected_output_re = 'Could not delete.*?a_table_that_does_not_exist.*'


# [[test]]
# name = "A transform can be created"
