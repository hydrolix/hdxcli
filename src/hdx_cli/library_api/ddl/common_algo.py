...
def ddl_datatype_to_hdx_datatype(data_mapping_file, ddl_name: str) -> DdlTypeToHdxTypeMappingFunc:
    ...
    def data_converter_func(ddl_datatype: Any) -> Union[str, List[str]]:
        ...
        try:
            ...
            return composed_parser.parse(
                ddl_datatype, simple_datatypes_mapping, compound_datatypes_mapping
            )
...
def _select_potential_primary_key_candidates_maybe_interactive(
    ...
    if cti.default_primary_key and cti.candidate_primary_keys != 1:
        ...
        return choose_interactively(
            f"Please choose the primary key for your transform. "
...
def _select_delimiter(_: DdlCreateTableInfo):
    return choose_interactively("Please choose the csv delimiter (default is ','): ", default=",")
...
class GenericPostProcessingHook(PostProcessingHook):
    def post_process(
        self, ddl_create_table_info: DdlCreateTableInfo, user_choices_dict: Dict[str, Any]
    ):
        ...
        if user_choices_dict:
            for user_choice in ALL_USER_CHOICE_KEYS:
                ...
                field_val, already_set_choices[user_choice] = _try_apply_user_choice_from_dict(
                    user_choices_dict, user_choice
                )
            ...
            if ddl_create_table_info.ingest_type == "csv":
                ddl_create_table_info.csv_delimiter = (
                    user_choices_dict.get("csv_delimiter")
                    if user_choices_dict.get("csv_delimiter") is not None
                ...
                if (indexes := user_choices_dict.get("csv_indexes")) is not None:
                    ...
                    ddl_create_table_info.csv_input_indexes = USER_CHOICE_FUNCS["csv_indexes"](
                        ddl_create_table_info
                    )
                    already_set_choices["csv_indexes"] = True
        ...
        if ddl_create_table_info.ingest_type == "csv":
            if not already_set_choices["csv_delimiter"]:
                ddl_create_table_info.csv_delimiter = USER_CHOICE_FUNCS["csv_delimiter"](
                    ddl_create_table_info
                )
...
def ddl_to_create_table_info(
    source_mapping: str,
    ddl_name: str,
    ddl_to_hdx_mapping_func: DdlTypeToHdxTypeMappingFunc,
    *,
    ...
    source_to_ti_proc: SourceToTableInfoProcessor = globals()[
        f"{ddl_name.lower().capitalize()}SourceToTableInfoProcessor"
    ]()
    ...
    for column_or_table_and_project_name in source_to_ti_proc.yield_table_info_tokens(
        ...
        if isinstance(column_or_table_and_project_name, ColumnDefinition):
            if (
                column_or_table_and_project_name.hdx_datatype in ("datetime", "epoch")
                and not column_or_table_and_project_name.column_comes_from_object_field
            ):
    ...
    if user_choices_file:
        with open(user_choices_file, encoding="utf-8") as user_choices_stream:
            user_choices_dict = json.load(user_choices_stream)
    GenericPostProcessingHook().post_process(create_tbl_info, user_choices_dict)
    try:

        globals()[ddl_name.lower().capitalize() + "PostProcessingHook"]().post_process(
            create_tbl_info, user_choices_dict.get(ddl_name.lower())
        )
    except KeyError:
        pass
...
def _needs_index(col_def: ColumnDefinition, ddl: DdlCreateTableInfo):
...
def _create_transform_elements_for(col_def: ColumnDefinition):
    ...
    for typename in hdx_datatype_raw[1:]:
        dict_result = {"type": typename, "index": typename != "double", "denullify": True}
        the_default: Optional[Any] = None
        if typename.startswith("int") or typename.startswith("uint") or typename == "double":
            the_default = 0
...
def _create_transform_output_column(
    ...
    *,
    ...
    datatype["type"] = (
        hdx_datatype_raw if not isinstance(hdx_datatype_raw, list) else hdx_datatype_raw[0]
    )
    ...
    *,
    ...
    for col in ddl.columns:
        ...
        output_columns.append(
            _create_transform_output_column(
                col,
                ddl,
