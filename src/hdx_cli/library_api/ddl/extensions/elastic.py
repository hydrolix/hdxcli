...
__all__ = ["ElasticSourceToTableInfoProcessor", "ElasticComposedTypeParser"]


# pylint: disable=R0903
class ElasticComposedTypeParser(ComposedTypeParser):
    def parse(
        self, ddl_datatype, simple_datatypes_mapping, compound_datatypes_mapping
    ) -> List[str]:
        pass
...
def _do_parse_elastic_object_type(base_name, value, results):
    if the_type := value.get("type"):
        ...
        for prop_name, prop_value in value["properties"].items():
            _do_parse_elastic_object_type(base_name + "__" + prop_name, prop_value, results)
...
def _parse_elastic_object_type(base_name, value, mapper):
    ...
    for field_name, datatype in results:
        if (mapped_datatype := mapper(datatype)) is not None:
            ...
            yield ColumnDefinition(datatype=datatype, identifier=field_name, ignored_field=True)


# pylint: disable=R0903
class ElasticSourceToTableInfoProcessor(SourceToTableInfoProcessor):
    def yield_table_info_tokens(
        self, source_mapping, mapper
        ...
        for prop_name, prop_value in elastic_mappings["properties"].items():
            if prop_type := prop_value.get("type"):
                if prop_type == "alias":
                    yield ColumnDefinition(
                        datatype=prop_type, identifier=prop_name, ignored_field=True
                    )
                else:
                    yield ColumnDefinition(
                        datatype=prop_type,
        ...
        if user_choices_dict and (array_fields := user_choices_dict.get("array_fields")):
            for field in array_fields:
                try:
                    ...
                    logger.warning(
                        f"WARNING: ignored inexistent field {field} in elastic.array_fields"
                    )
            used_choices["array_fields"] = True
        if not used_choices.get("array_fields"):
            the_cols = set(
                {c for c in ddl_create_table_info.columns if not c.column_comes_from_object_field}
            )
