...
from hdx_cli.library_api.common.validation import is_valid_hostname, is_valid_username
...
def get_profile_data_from_standard_input(
    hostname: Optional[str] = None,
    username: Optional[str] = None,
    http_scheme: Optional[str] = None,
) -> Optional[ProfileWizardInfo]:
    input_hostname = None
    good_hostname = False
    input_username = None
    good_username = False
    ...
    if authorization_file.exists():
        try:
            ...
            raise HdxCliException(
                f"Error deleting the authorization file for '{profile_name}' profile."
            ) from exc


def delete_profile(
    profile_name: str, initial_profile: dict, *, profile_config_file: Path = PROFILE_CONFIG_FILE
):
    try:
        del initial_profile[profile_name]
    except KeyError as exc:
        raise HdxCliException(
            f"There was an error trying to delete '{profile_name}' profile."
        ) from exc
