...
__all__ = ["choose_interactively", "choose_from_elements_interactively"]

_SHOW_MAX_ITEMS = 10


def choose_interactively(
    prompt, *, default: Optional[str] = None, valid_choices: Optional[Sequence[Any]] = None
) -> str:
    choice = None
    ...
    while not choice or choice not in valid_choices:
        ...
        for conv_type in (int, float):
            try:
                valid_choices_failed = list(
                    map(str, sorted(conv_type(x) for x in valid_choices_failed))
                )
                break
            except ValueError:
                pass
        valid_choices_str = (
            ", ".join(valid_choices_failed)
            if len(valid_choices_failed) < _SHOW_MAX_ITEMS
            else ", ".join(valid_choices_failed[0 : _SHOW_MAX_ITEMS // 2])
            + "..."
