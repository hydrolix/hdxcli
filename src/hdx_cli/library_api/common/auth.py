...
from .context import ProfileLoadContext, ProfileUserContext
from .exceptions import CacheFileNotFoundException, LogicException, ProfileNotFoundException

__all__ = ["load_profile", "try_load_profile_from_cache_data", "save_profile_cache"]


@overload
def load_profile(load_profile_context: ProfileLoadContext) -> ProfileUserContext: ...
...
def load_profile(load_profile_context: Union[ProfileLoadContext, str]) -> ProfileUserContext:
    ...
    try:
        ...
        if isinstance(load_profile_context, ProfileLoadContext):
            profile_config_file = (
                load_profile_context.profile_config_file
        ...
        raise ProfileNotFoundException(f"File not found: {profile_config_file}") from ex
    except KeyError as key_err:
        raise ProfileNotFoundException(f"Profile name not found: {profile_name}") from key_err


def save_profile_cache(
    a_profile: ProfileUserContext,
    *,
    token,
...
def try_load_profile_from_cache_data(load_ctx: ProfileLoadContext) -> ProfileUserContext:
    ...
    user_dict = {
        ...
        "auth": AuthInfo(
            cache["token"]["auth_token"], cache["token"]["expires_at"], cache["org_id"]
        ),
        "org_id": cache["org_id"],
