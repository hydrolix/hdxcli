...
@click.command()
@click.argument("policy_name", metavar="ROW_POLICY_NAME")
@click.option(
    "--filter",
    "filter_expression",
...
def create(
    ...
    """Creates a new row policy for the specified table.
    A row policy filters the data that users can see based on a filter
    expression. It must be associated with at least one role to take effect.

    \b
    Examples:
      # Create a PERMISSIVE policy to show logs from Europe to 'analyst' users
      hdxcli row-policy --project hydro --table logs create europe_logs --filter "region = 'EU'" --role analyst

    \b
      # Create a RESTRICTIVE policy to ensure only non-draft documents are ever shown
    ...
    basic_create(user_profile, resource_path, policy_name, body=body)
    logger.info(f"Created row policy {policy_name}")
...
def list_(ctx: click.Context):
    """Lists all row policies for a given table.
    Displays a summary of all policies, including their name, filter
    expression, whether they are restrictive, and their associated roles.

    \b
    Examples:
      # List all policies for the 'logs' table in the 'hydro' project
      hdxcli row-policy --project hydro --table logs list
    """
    user_profile = ctx.parent.obj["usercontext"]
    ...
    if not policies:
        ...
        logger.info(f"No row policies found for table '{project}.{table}'.")
        return
...
def _role_operation(
    ctx: click.Context, policy_name: str, roles: list[str], operation: str
):
...
@click.pass_context
@report_error_and_exit(exctype=Exception)
def add_role(ctx: click.Context, policy_name: str, roles: list[str]):
    """Adds one or more roles to an existing row policy.
    This command associates roles with a policy, granting the
    permissions defined by that policy to users who have those roles.

    \b
    Examples:
      # Add the 'viewer' role to the 'europe_logs' policy
    ...
    """
    _role_operation(ctx, policy_name, roles, "add_roles")
...
@click.pass_context
@report_error_and_exit(exctype=Exception)
def remove_role(ctx: click.Context, policy_name: str, roles: list[str]):
    """Removes one or more roles from an existing row policy.
    This command disassociates roles from a policy, revoking the
    permissions defined by that policy from users who have those roles.
    ...
    """
    _role_operation(ctx, policy_name, roles, "remove_roles")
