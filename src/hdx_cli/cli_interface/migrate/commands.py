...
def _setup_target_cluster_config(
    profile_config_file, target_cluster_username, target_cluster_hostname, target_cluster_scheme
):
...
def migrate_projects(
    ...
    source_projects, source_projects_url = access_resource_detailed(
        source_profile, [("projects", None)]
    )
    ...
    for project in source_projects:
        ...
        try:
            show_result = json.loads(
                basic_show(source_profile, source_resource_path, project["name"])
            )
...
def create_tables_for_project(
    project_name, source_profile: ProfileUserContext, target_profile: ProfileUserContext
):
    source_project_tables, source_tables_url = access_resource_detailed(
        source_profile, [("projects", project_name), ("tables", None)]
    )
    ...
    for table in source_project_tables:
        ...
        if table.get("type") and table.get("type") == "summary":
            ...
            try:
                table_show = json.loads(
                    basic_show(source_profile, source_tables_path, table["name"])
                )
                try:
                    ...
                    basic_create_with_body_from_string(
                        target_profile, target_tables_path, table["name"], json.dumps(table_show)
                    )
            except HttpException as exc:
                if exc.error_code == 400:
...
def create_postponed_tables(
    project_name, source_profile: ProfileUserContext, target_profile: ProfileUserContext, table
):
    source_project_tables, source_tables_url = access_resource_detailed(
        source_profile, [("projects", project_name), ("tables", None)]
    )
...
def create_functions_for_project(
    project_name, source_profile: ProfileUserContext, target_profile: ProfileUserContext
):
    source_project_functions, source_functions_url = access_resource_detailed(
        source_profile, [("projects", project_name), ("functions", None)]
    )
    ...
    for function in source_project_functions:
        try:
            function_show = json.loads(
                basic_show(source_profile, source_functions_path, function["name"])
            )
...
def create_dictionaries_for_project(
    project_name, source_profile: ProfileUserContext, target_profile: ProfileUserContext
):
    source_project_dictionaries, _ = access_resource_detailed(
        source_profile, [("projects", project_name), ("dictionaries", None)]
    )
    _, target_project_url = access_resource_detailed(target_profile, [("projects", project_name)])
    ...
    for dic in source_project_dictionaries:
        ...
        try:
            if d_file not in dictionary_files_so_far:
                _create_dictionary_file_for_project(project_name, d_file, contents, target_profile)
            ...
            try:
                basic_create_with_body_from_string(
                    target_profile, target_dict_path, d_name, json.dumps({"settings": d_settings})
                )
            except HttpException as exc:
                if exc.error_code == 400:
                    yield dic, MigrateStatus.SKIPPED
...
def _create_dictionary_file_for_project(
    project_name, dict_file, contents, profile: ProfileUserContext
):
...
def create_transforms_for_table(
    project_name, table_name, source_profile: ProfileUserContext, target_profile: ProfileUserContext
):
    source_table_transforms, source_transforms_url = access_resource_detailed(
        source_profile, [("projects", project_name), ("tables", table_name), ("transforms", None)]
    )

    _, target_table_url = access_resource_detailed(
        target_profile, [("projects", project_name), ("tables", table_name)]
    )
    ...
    for transform in source_table_transforms:
        transform_show = json.loads(
            basic_show(source_profile, source_transforms_path, transform["name"])
        )
        try:
...
@click.command(
    help="Migrate projects to a target cluster. The migrate command takes care of migrating"
...
@click.argument(
    "target_cluster_username", metavar="TARGET_CLUSTERUSERNAME", required=True, default=None
)
@click.argument(
    "target_cluster_hostname", metavar="TARGET_CLUSTERHOSTNAME", required=True, default=None
)
...
def migrate(
    ...
    if project_blacklist and project_whitelist:
        raise LogicException(
            "You can only use project whitelist or project black list but not both."
        )
    ...
    target_user_profile = generate_temporal_profile(
        target_cluster_hostname,
        target_cluster_username,
        target_cluster_password,
        target_cluster_uri_scheme,
    ...
    with mrm(
        target_user_profile,
    ) as migration_rollback_manager:
        postponed_tables = []
        for project, status in migrate_projects(
            ctx,
            ...
            if status == MigrateStatus.CREATED:
                logger.info("created")
                m_entry = MigrationEntry(project["name"], ResourceKind.PROJECT)
                migration_rollback_manager.push_entry(m_entry)
            elif status == MigrateStatus.SKIPPED:
                logger.info("skipped creation (was found)")

            for func, func_status in create_functions_for_project(
                project["name"], ctx.parent.obj["usercontext"], target_user_profile
            ):
                logger.info(f'\tFunction {func["name"]}: [!n]')
                if func_status == MigrateStatus.CREATED:
                    logger.info("created")
                    m_entry = MigrationEntry(func["name"], ResourceKind.FUNCTION, [project["name"]])
                    migration_rollback_manager.push_entry(m_entry)
                elif func_status == MigrateStatus.SKIPPED:
                    logger.info("skipped creation (was found)")
            for dictionary, dict_status in create_dictionaries_for_project(
                project["name"], ctx.parent.obj["usercontext"], target_user_profile
            ):
                logger.info(f'\tDictionary {dictionary["name"]}: [!n]')
                if dict_status == MigrateStatus.CREATED:
                    logger.info("created")
                    m_entry = MigrationEntry(
                        dictionary["name"], ResourceKind.DICTIONARY, [project["name"]]
                    )
                    migration_rollback_manager.push_entry(m_entry)
                elif dict_status == MigrateStatus.SKIPPED:
                    logger.info("skipped creation (was found)")

            for table, tbl_status in create_tables_for_project(
                project["name"], ctx.parent.obj["usercontext"], target_user_profile
            ):
                logger.info(f'\tTable {table["name"]}: [!n]')
                if tbl_status == MigrateStatus.CREATED:
                    logger.info("created")
                    m_entry = MigrationEntry(table["name"], ResourceKind.TABLE, [project["name"]])
                    migration_rollback_manager.push_entry(m_entry)
                ...
                for transform, transform_status in create_transforms_for_table(
                    project["name"],
                    table["name"],
                    ctx.parent.obj["usercontext"],
                    target_user_profile,
        ...
        if postponed_tables:
            ...
            for table, project_name in postponed_tables:
                tbl_status = create_postponed_tables(
                    project_name, ctx.parent.obj["usercontext"], target_user_profile, table
                )
                logger.info(f'\tProject {project_name}. Table {table["name"]}: [!n]')
                if tbl_status == MigrateStatus.CREATED:
                    logger.info("created")
                    m_entry = MigrationEntry(table["name"], ResourceKind.TABLE, [project_name])
                    migration_rollback_manager.push_entry(m_entry)
                elif tbl_status == MigrateStatus.SKIPPED:
                    logger.info("skipped creation (was found)")
